# Hi 👋, I'm Polam Arunteja

🎓 B.Tech Student | AI & ML  
🐍 Python | DSA | Machine Learning  
💻 Aspiring Software & AI/ML Developer

## 👨‍💻 About Me

- 🎓 B.Tech student specializing in Artificial Intelligence & Machine Learning
- 🐍 Currently strengthening my Python and DSA skills
- 🤖 Interested in Machine Learning, Artificial Intelligence and Software Development
- 🚀 Building practical projects to solve real-world problems
- 📚 Preparing for technical placements

## 🛠️ Skills

### Programming
- Python
- C++
- Java

### AI / ML
- Machine Learning
- Data Science
- Python for Data Science

### Web Development
- React
- JavaScript
- HTML
- CSS
- Tailwind CSS

### Tools
- Git
- GitHub
- VS Code

## 🚀 Projects

### 🏞️ LandVerify AI
AI-powered land record intelligence and cadastral validation system.

**Tech:** React, JavaScript, Tailwind CSS, Leaflet, AI/OCR

### 👁️ Edge AI Proctoring System
AI-based online examination monitoring system using computer vision.

**Tech:** React, JavaScript, MediaPipe

### 📄 Automated Resume Analyzer
Resume analysis and job matching system using NLP and Python.

**Tech:** Python, FastAPI, NLP

## 📚 Currently Learning

- Data Structures & Algorithms
- Python
- Machine Learning
- AI
- Problem Solving

## 🤝 Connect With Me

- 💼 LinkedIn: [Polam Arunteja](https://www.linkedin.com/in/polam-arun-teja-39018440b/)

---

⭐ Thanks for visiting my profile!

<!--
**arunteja25/arunteja25** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
